<template>
  <transition :name="transitionName">
    <div v-show="visible" @click.self="handleMaskClose" class="n-drawer-warpper">
      <div :class="['n-drawer', 'transition-inner', `n-drawer-${direction}`]"
        :style="isDirection ? `width: ${size}` : `height: ${size}`">
        <div>
          <div class="n-drawer-header">
            <span class="n-drawer-title" v-if="showHeader">
              {{ title }}
            </span>
            <slot v-else name="headerContent" />
            <Icon v-if="closable" @click="onClickCancelButton" class="n-drawer-close" name="close" />
          </div>
        </div>
        ...
        <div>
          ...
          <div class="n-drawer-footer" v-if="showFooter">
...
const transitionName = computed((): string => {
  return `n-drawer-fade-${props.direction}`;
});
