<template>
  <div class="n-upload-dragger" :class="{ ['n-upload-draggerenter']: isEnter }" ref="fileArea"
    @click="emits('fileUpload')">
    <div class="n-upload-content">
      <Icon class="n-upload-icon" :name="isEnter ? 'folder-open' : 'folder'" />
      <div class="n-upload-dragger-text">将文件拖到此处或<em>点击上传</em></div>
    </div>
  </div>
</template>
