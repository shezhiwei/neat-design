<template>
  <div class="n-popover-box" @mouseenter="hoverTriggerEnterHandler" @mouseleave="hoverTriggerLeaveHandler">
    <transition name="fade" @after-enter="handleAfterEnter" @after-leave="handleAfterLeave">
      <div v-show="!disabled && showPopover" :class="['n-popover-outbox', placement, popperClass]"
        :aria-hidden="disabled || !showPopover ? 'true' : 'false'">
        <div class="n-popover-arrow"></div>
        <div :class="['n-popover-box-content']" :style="popoverStyles">
          <div v-if="title" v-text="title" class="n-popover-title"></div>
          <slot>{{ content }}</slot>
        </div>
      </div>
    </transition>
    <div class="n-reference-content" @click="clickTriggerHandler" @mousedown="focusTriggerHandler"
