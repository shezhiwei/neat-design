<template>
  ...
  <div class="n-table" :style="{ height }">
    <table class="n-origin-table" border="0" cellspacing="0" cellpadding="0" :width="width">
      <thead class="n-thead">
        <tr class="n-thead-th" :height="trHeight">
          <th v-if="rowSelect" style="width: 30px; text-align: center">
            <input ref="checkAllBox" type="checkbox" :checked="isAllChecked" @change="selectAllChange($event)" />
          </th>
          <th v-if="num" :class="[`n-thead-td`, isBorder()]" :align="align || 'left'" />
          <th v-for="(item, i) in columns" :key="i" :class="[`n-thead-td`, isBorder()]" :width="item.width"
            :align="align || 'left'">
            {{ item.title }}
          </th>
        </tr>
      ...
      <tbody class="n-tbody">
        <tr v-for="(dataItem, ind) in (data as any)" :key="ind" :class="[
          'n-tbodn-tr',
          {
            'n-tbodn-tr-zebra': zebra,
          },
        ]" :height="trHeight" :style="importantStyle(ind)">
          <td v-if="rowSelect" style="width: 30px; text-align: center">
            <input type="checkbox" @change="selectItemsChange(dataItem, $event)" :checked="isRowChecked" />
          </td>
          <td v-if="num" :class="[`n-tbodn-td`, isBorder()]" :align="align || 'left'">
            {{ ind + 1 }}
          </td>
          <td v-for="(colItem, index) in (columns as any)" :key="index" :style="{ width: `${colItem.width * 10}px` }"
            :class="[`n-tbodn-td`, isBorder()]" :align="align || 'left'">
...
const isBorder = (): object => {
  return { 'n-table-border': props.border };
};
