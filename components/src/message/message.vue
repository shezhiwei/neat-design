<template>
  <transition @before-leave="onClose" @after-leave="onDestroy" name="n-message-fade">
    <div v-show="visiable" :class="messageStyle" :style="{ top: `${top}px` }">
      <Icon class="before-icon" v-if="iconType" :name="icon || iconType[type]" />
      <div class="n-message-content" v-html="message"></div>
...
const messageStyle = computed(() => ['n-message', props.type]);
