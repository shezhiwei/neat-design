<template>
  <div :class="selectClass" :style="{ width: sizeChange(width) }" v-click-outside>
    <div :class="selectInputClass">
      <input type="text" :readonly="!searchable || multiple" :placeholder="stashPlaceholder" :disabled="disabled"
        @input="handleInput" :value="selectedLabel" />
      <div class="select-icon">
        <Icon name="caret-down" class="rorateIcon" :style="{ transform: rotate }"
          :icon-style="{ width: '13px', height: '13px' }" />
      </div>
    </div>
    <transition name="slide-fade">
      <div class="n-select-option" v-if="isShow">
        <div class="n-select-option-find">
          <div class="n-arrow" />
          <ul>
            <li class="n-select-option-li" v-for="item in optionsData" :key="item[fieldValue]" @click="selChange(item)"
              :class="{
                'n-select-active': itemSelected(item[fieldValue]),
                'n-select-disabled': item.disabled,
              }">
              {{ item[fieldLabel] }}
              <Icon v-if="multiple && itemSelected(item[fieldValue])" name="check"
...
const selectClass = computed(() => {
  return ['n-select', { 'n-select-disabled': props.disabled }];
});
const selectInputClass = computed(() => {
  return ['n-select-input', { 'n-select-input-disabled': props.disabled }];
});
...
const vClickOutside = {
  beforeMount(el: HTMLElementPlus) {
    let handler = (e: Event) => {
      ...
      //点击的目标不在选项上
      const notInPopup = (e.target as HTMLElement).className?.indexOf('n-select-option-li') == -1;
      if (!props.multiple) {
        // 单选处理
