<template>
  <transition @before-leave="onClose" @after-leave="onDestroy" name="n-notification-fade">
    <div v-show="visiable" class="n-notification" :style="{ top: `${top}px` }">
      <div class="before-icon" v-if="type || icon">
        <Icon size="22" :color="color || iconType[type as string]?.color"
          :name="icon || iconType[type as string]?.name" />
      </div>

      <div class="n-notification-content">
        <div class="n-notification-content-title">{{ message }}</div>
        <div class="n-notification-content-description" v-if="description">
          {{ description }}
        </div>
      </div>
